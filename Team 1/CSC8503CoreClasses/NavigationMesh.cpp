...
NavigationMesh::NavTri* NavigationMesh::GetTriForPosition(const Vector3& pos) {
	...
	for (NavTri& t : allTris) {
		...
		float dist = abs(areaSum - t.area);

		if (dist < 0.01f && (!closestTri || dist < smallestDist)) { //floating points are annoying! Are we more or less inside the triangle?
			closestTri = &t;
			smallestDist = dist;
		}
	}
...
NavigationPath NavigationMesh::StringPull(std::vector<NavTri*> path, Vector3 startPoint, Vector3 endPoint) {
	...
	std::vector<NavTri*> realPath = path.size() > 1 ? InvertPath(path) : path;
