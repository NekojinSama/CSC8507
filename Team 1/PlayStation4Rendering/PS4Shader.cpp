...
void PS4Shader::GenerateFetchShader(char* binData) {
	fetchShader = GarlicAllocator.allocate(
	Gnmx::Toolkit::calculateMemoryRequiredForVsFetchShader(binData));
	
	uint32_t shaderModifier;
	Gnmx::generateVsFetchShader(fetchShader, &shaderModifier,
	vertexShader, NULL);
