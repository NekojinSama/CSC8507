...
APILoadDataFunction TextureLoader::apiDataFunction = nullptr;
APIBindImageFunction TextureLoader::apiImageFunction = nullptr;
...
void TextureLoader::RegisterAPILoadDataFunction(APILoadDataFunction f) {
	if (apiDataFunction) {
		std::cout << __FUNCTION__ << " replacing previously defined API texture from data function.\n";
	}
	apiDataFunction = f;
}
void TextureLoader::RegisterAPIBindImageFunction(APIBindImageFunction f) {
	if (apiImageFunction) {
		std::cout << __FUNCTION__ << " replacing previously defined API image creation function.\n";
	}
	apiImageFunction = f;
}
...
TextureBase* TextureLoader::LoadAPIDataTexture(char* data, int width, int height, int channels) {
	if (apiDataFunction == nullptr) {
		std::cout << __FUNCTION__ << " no API mask Function has been defined!\n";
		return nullptr;
	}
	return apiDataFunction(data, width, height, channels);
}
TextureBase* TextureLoader::BindAPITextureImage(TextureBase* texture) {
	if (apiImageFunction == nullptr) {
		std::cout << __FUNCTION__ << " no API image creation Function has been defined!\n";
		return nullptr;
	}
	return apiImageFunction(texture);
}