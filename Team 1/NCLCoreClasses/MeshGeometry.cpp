...
MeshGeometry* MeshGeometry::GenerateQuad(MeshGeometry* input) {
	input->SetVertexPositions({ Vector3(-1,1,0), Vector3(-1,-1,0), Vector3(1,1,0), Vector3(1,-1,0)});
	input->SetVertexColours({ Vector4(1,1,1,1), Vector4(1,1,1,1), Vector4(1,1,1,1), Vector4(1,1,1,1)});
	input->SetVertexTextureCoords({ Vector2(0,1), Vector2(0,0), Vector2(1, 1),Vector2(1, 0) });
	input->SetVertexIndices({ 0,1,2,2,3,0 });
	input->debugName = "Test-Quad";
	return input;
}
