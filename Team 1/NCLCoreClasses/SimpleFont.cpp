...
void SimpleFont::BuildVerticesForString(const std::string& text, const Vector2& startPos, const Vector4& colour, Vector2 size, std::vector<Vector3>&positions, std::vector<Vector2>&texCoords, std::vector<Vector4>&colours) {
	...
	for (size_t i = 0; i < text.length(); ++i) {
		...
		float charWidth  = (float)((charData.x1 - charData.x0)/ texWidth) * scale.x;
		float charHeight = (float)(charData.y1 - charData.y0);

		float xStart	= ((charData.xOff + currentX) * texWidthRecip) * scale.x;
		float yStart	= startPos.y;
		float yHeight	= (charHeight * texHeightRecip) * scale.y;
		float yOff		= ((charHeight + charData.yOff) * texHeightRecip) * scale.y;
