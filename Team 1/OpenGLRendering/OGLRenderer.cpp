...
OGLRenderer::OGLRenderer(Window& w) : RendererBase(w)	{
	...
	if (initState) {
		...
		TextureLoader::RegisterAPILoadDataFunction(OGLTexture::RGBATextureFromData);
		TextureLoader::RegisterAPIBindImageFunction(OGLTexture::BindTextureImage);
	}
...
void OGLRenderer::RenderFrame(int type)		{
